...
struct TwitItemView: View {
	//			MARK: - Properties

	let index: Int = .random(in: 1 ... 13)
...
//						username
						HStack(spacing: 05) {
							...
							Text("@\(twitUser.username)")
								.lineLimit(1)
								.foregroundStyle(.secondary)
								.font(Font.custom(AppFontsFamily.regular.name, size: 16))
							Text("·3h")
								.foregroundStyle(.secondary)
...
//						show thread
			...
			.shadow(color: Color(red: 0.81, green: 0.84, blue: 0.86), radius: 0, x: 0, y: 0.33)
//			.shadow(color: Color.navBarShadow, radius: 0.33, x: 0, y: 0)
